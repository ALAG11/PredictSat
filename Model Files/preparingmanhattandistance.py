...
for filename in os.listdir(dataset_dir):
    ...
    if filename.endswith('.png') or filename.endswith('.jpg'):
        ...
        # Transforming the data to use Manhattan distance
        transformed_pixels = np.hstack([pixels, pixels])

        # Performing MiniBatchKMeans clustering (Object / Feature Detection)
        kmeans = MiniBatchKMeans(n_clusters=50, random_state=0).fit(transformed_pixels)

        # Replace each pixel with its cluster center
        segmented_img = kmeans.cluster_centers_[kmeans.labels_].reshape(normalized_img.shape + (2,))[:, :, 0].astype(np.uint8)
